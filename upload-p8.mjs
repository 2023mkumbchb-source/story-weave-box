...
const dir = 'cropped-images/anatomy-spot-revision';
...
async function main() {
  ...
  for (const file of files) {
    const url = await uploadOne(path.join(dir, file), `y1-anatomy-anatomy-spot-revision-${file}`);
    mapping[`anatomy-spot-revision/${file}`] = url;
    process.stdout.write('.');
  }
  console.log('\ndone', Object.keys(mapping).length);
  fs.writeFileSync('r2-anatomy-spot-revision-mapping.json', JSON.stringify(mapping, null, 2));
}
main();
