...
const dir = 'cropped-images/limb-anatomy-revision';
...
async function main() {
  ...
  for (const file of files) {
    const url = await uploadOne(path.join(dir, file), `y1-anatomy-limb-anatomy-revision-${file}`);
    mapping[`limb-anatomy-revision/${file}`] = url;
    process.stdout.write('.');
  }
  console.log('\ndone', Object.keys(mapping).length);
  fs.writeFileSync('r2-limb-anatomy-revision-mapping.json', JSON.stringify(mapping, null, 2));
}
main();
