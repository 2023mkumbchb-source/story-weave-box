...
const dir = 'cropped-images/pat8-2012';
...
async function main() {
  ...
  for (const file of files) {
    const url = await uploadOne(path.join(dir, file), `y1-anatomy-pat8-2012-${file}`);
    mapping[`pat8-2012/${file}`] = url;
    process.stdout.write('.');
  }
  console.log('\ndone', Object.keys(mapping).length);
  fs.writeFileSync('r2-pat8-2012-mapping.json', JSON.stringify(mapping, null, 2));
}
main();
