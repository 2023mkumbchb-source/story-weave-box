...
const dir = 'cropped-images/week-12-review';
...
async function main() {
  ...
  for (const file of files) {
    const url = await uploadOne(path.join(dir, file), `y1-anatomy-week-12-review-${file}`);
    mapping[`week-12-review/${file}`] = url;
    process.stdout.write('.');
  }
  console.log('\ndone', Object.keys(mapping).length);
  fs.writeFileSync('r2-week-12-review-mapping.json', JSON.stringify(mapping, null, 2));
}
main();
