...
const dir = 'cropped-images/histology-embryology-spot-review';
...
async function main() {
  ...
  for (const file of files) {
    const url = await uploadOne(path.join(dir, file), `y1-anatomy-histology-embryology-spot-review-${file}`);
    mapping[`histology-embryology-spot-review/${file}`] = url;
    process.stdout.write('.');
  }
  console.log('\ndone', Object.keys(mapping).length);
  fs.writeFileSync('r2-histology-embryology-spot-review-mapping.json', JSON.stringify(mapping, null, 2));
}
main();
